...
using ..NodeModule: AbstractScalarExprNode, with_type_parameters, tree_mapreduce
using ..EvaluateModule: eval_tree_array
using ..EvaluateDerivativeModule: eval_grad_tree_array

struct NodeTangent{T,N<:AbstractScalarExprNode{T},A<:AbstractArray{T}} <: AbstractTangent
    tree::N
    gradient::A
end
function extract_gradient(gradient::NodeTangent, ::AbstractScalarExprNode)
