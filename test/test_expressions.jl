...
@testitem "Expression Literate examples" begin
    ...
    `AbstractScalarExprNode` (typically a `Node`) with metadata like operators
    and variable names.
    =#
    ...
    and so there are no scope issues as with raw `AbstractScalarExprNode` types
    ...
    # We can convert an expression into the primitive `AbstractScalarExprNode` type
    # with [`get_tree`](@ref):
    tree = get_tree(parsed_expr)
    @test tree isa AbstractScalarExprNode  #src
    ...
    xs[1] + xs[2]
    # These have the same type – they simply combine their `AbstractScalarExprNode` objects
