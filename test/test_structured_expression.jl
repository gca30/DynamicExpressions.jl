...
@testitem "StructuredExpression Literate examples" begin
    ...
    expression that exists outside of the regular `AbstractScalarExprNode` objects
    ...
    *stored* with the `+` operator in an `AbstractScalarExprNode`.
